...
use cosmwasm_std::{
    to_json_binary, BalanceResponse, BankMsg, Binary, Decimal, Coin, CosmosMsg, Deps, DepsMut, Env,
...
pub fn instantiate(
    ...
    if amount != Uint128::from(1000_u64)
    {
...
pub fn try_borrow(
    ...
    if amount > available_to_borrow {
        return Err(ContractError::BorrowExceeded { available: available_to_borrow});
    }
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetDebt { address } => to_json_binary(&query_debt(deps, address)?),
    }
}
